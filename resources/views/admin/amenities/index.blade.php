        ...
        <div class="card-body">
            @if(adminAccessRoute(config('role.manage_property.access.add')))
                <div class="media mb-4 justify-content-end">
                    @if(adminAccessRoute(config('role.manage_property.access.add')))
                        <a href="{{route('admin.amenitiesCreate')}}" class="btn btn-sm btn-primary btn-rounded mr-2">
                        ...
                        @forelse($amenities as $item)
                            <tr>
                                ...
                                <td data-label="@lang('Status')">
                                    <span class="custom-badge {{ @$item->status == 1 ? 'bg-success' : 'bg-danger' }}">@lang(@$item->status == 1 ? 'Active' : 'Deactive')</span>
                                </td>
                                @if(adminAccessRoute(config('role.manage_property.access.edit')) == true || adminAccessRoute(config('role.manage_property.access.delete')) == true)
                                <td data-label="@lang('Action')">
                                    @if(adminAccessRoute(config('role.manage_property.access.edit')))
                                        <a href="{{ route('admin.amenitiesEdit',$item->id) }}" class="btn btn-sm btn-outline-primary btn-rounded edit-button">
